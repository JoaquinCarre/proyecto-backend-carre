...
window.addEventListener('load', async () => {
  const userLog = await fetch("http://localhost:8010/users/me");
...
formAuth.addEventListener('submit', async (event) => {
  ...
  if (event.submitter.id === 'signin-button') {
    responseFetch = await fetch("http://localhost:8010/auth/sign-in", {
      headers: {
    ...
    responseFetch = await fetch("http://localhost:8010/auth/sign-up", {
      headers: {
        'Content-Type': 'application/json',
        'Content-Length': dataJSON.length
      },
...
SignOutButton.addEventListener('click', async () => {
  formAdd.classList.add('d-none');
  const userOut = await fetch("http://localhost:8010/auth/sign-out", {
    headers: {
      'Content-Type': 'application/json'
    },
    method: 'POST',
